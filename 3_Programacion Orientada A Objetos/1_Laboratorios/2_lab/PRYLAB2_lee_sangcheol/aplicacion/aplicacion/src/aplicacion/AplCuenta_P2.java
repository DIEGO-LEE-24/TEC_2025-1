...
/**
 * Clase AplCuenta_P2 – Segunda Parte
 * 
 * En esta versión se completa la funcionalidad de la aplicación, implementando
 * los métodos retirar() y verSaldo() para permitir transacciones completas.
 */
public class AplCuenta_P2 {
    ...
    // Implementación completa del método retirar()
    static void retirar(int pindice) throws IOException {
        ...
        if (saldos[pindice] >= monto) {
            saldos[pindice] -= monto;
            out.println("Ha retirado " + monto + " colones exitosamente.");
        } else {
            out.println("Saldo insuficiente. No se puede realizar la transacción.");
        }
    }

    // Implementación completa del método verSaldo()
    static void verSaldo(int pindice) throws IOException {
        out.println("El saldo actual es de: " + saldos[pindice] + " colones.");
    }
}
